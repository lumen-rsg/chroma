...
/* VENDORED — unmodified copy. Required because wlr_layer_shell_v1.h
   includes it with a quoted #include and system wlroots may not install
   this generated file alongside the main header. */
...
zwlr_layer_surface_v1_error_is_valid(uint32_t value, uint32_t version) {
	...
	case ZWLR_LAYER_SURFACE_V1_ERROR_INVALID_KEYBOARD_INTERACTIVITY:
		return version >= 1;
	case ZWLR_LAYER_SURFACE_V1_ERROR_INVALID_EXCLUSIVE_EDGE:
		return version >= 1;
	default:
		return false;
	}
...
struct zwlr_layer_surface_v1_interface {
	 ...
	 *
	 * Requests an edge for the exclusive zone to apply. The
	 * exclusive edge will be automatically deduced from anchor points
	 * when possible, but when the surface is anchored to a corner, it
	 * will be necessary to set it explicitly to disambiguate, as it is
	 * not possible to deduce which one of the two corner edges should
	 * be used.
	 *
	 ...
	 * @since 5
	 */
	void (*set_exclusive_edge)(struct wl_client *client,
				   struct wl_resource *resource,
				   uint32_t edge);
};
