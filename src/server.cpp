...
bool WlrootsServer::init() {
    ...
    // Tearing control (for VRR/tearing in fullscreen games)
    if (!wlr_tearing_control_manager_v1_create(display, 1)) {
        std::fprintf(stderr, "Failed to create tearing-control manager\n");
        // non-fatal
    }
    ...
    // Idle notifier (for screensavers, idle detection)
    if (!wlr_idle_notifier_v1_create(display)) {
        std::fprintf(stderr, "Failed to create idle-notifier\n");
        // non-fatal
    }
    ...
    // Virtual keyboard (for on-screen keyboards, wtype, etc.)
    if (!wlr_virtual_keyboard_manager_v1_create(display)) {
        std::fprintf(stderr, "Failed to create virtual-keyboard-manager\n");
        // non-fatal
    }
